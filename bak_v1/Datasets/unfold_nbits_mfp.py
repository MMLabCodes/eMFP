...
from functions import mfp, clean_mols
...
for iter, bit in enumerate(bit_sizes):
    ...
    unique.append( x )
    finaltime = time.time()
    print('case,idx,bit,unique_bits,times,sumtime,mem_size')
    print(f'{input_file[:-4]},{idx_power[iter]},{bit},{x},{finaltime - begintime},{finaltime - initial_time},{np.array(fps, dtype = np.int8 ).nbytes}')
